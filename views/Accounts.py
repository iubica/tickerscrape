...
class AccountsModel(dv.DataViewIndexListModel):
    def __init__(self, log):
        dv.DataViewIndexListModel.__init__(self, Config.accountsDf.shape[0])
        self.log = log
    ...
    def GetValueByRow(self, row, col):
        ...
        if dataFrameCol is not None:
            value = str(Config.accountsDf.iloc[row, dataFrameCol])
    ...
    def ValidateValueByRow(self, value, row, col):
        if col == 0:
            ...
            for i in range(Config.accountsDf.shape[0]):
                if i == row:
                    continue
                if Config.accountsDf.iloc[i,0] == value:
    ...
    def GetRowCount(self):
        rowCount = Config.accountsDf.shape[0]
    ...
    def AddRow(self, id, value):
        ...
        Config.accountsDf.loc[id-1] = value
        # notify views
        self.RowAppended()
    ...
    def DeleteRows(self, rows):
        ...
        Config.accountsDf.drop(rows, inplace=True)
        # Reset the dataframe index, and don't add an index column
        Config.accountsDf.reset_index(inplace=True, drop=True)
    ...
    def MoveUp(self, rows):
        ...
        if rows:
            for row in rows:
                a = Config.accountsDf.iloc[row-1].copy()
