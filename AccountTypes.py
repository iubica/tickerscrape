...
class AccountTypesModel(dv.DataViewIndexListModel):
    def __init__(self, log):
        dv.DataViewIndexListModel.__init__(self, Config.accountTypesDf.shape[0])
        self.log = log
    ...
    def GetValueByRow(self, row, col):
        ...
        if dataFrameCol is not None:
            value = str(Config.accountTypesDf.iloc[row, dataFrameCol])
    ...
    def SetValueByRow(self, value, row, col):
        ...
        if dataFrameCol is not None:
            if col == 0:
                ...
                Config.accountTypesDf.iloc[row, dataFrameCol] = value
                Config.AccountTypesChanged(True)
    ...
    def GetRowCount(self):
        rowCount = Config.accountTypesDf.shape[0]
    ...
    def DeleteRows(self, rows):
        ...
        Config.accountTypesDf.reset_index(inplace=True, drop=True)
    ...
    def MoveUp(self, rows):
        ...
        if rows:
            for row in rows:
                a = Config.accountTypesDf.iloc[row-1].copy()
                b = Config.accountTypesDf.iloc[row].copy()
                Config.accountTypesDf.iloc[row-1] = b
                Config.accountTypesDf.iloc[row] = a
                Config.AccountTypesChanged(True)
...
class AccountTypesPanel(wx.Panel):
    def __init__(self, parent, log, model=None):
        ...
        if model is None:
            self.model = AccountTypesModel(log)
        else:
            self.model = model
...
def GetWindow(frame, nb, log):
    ...
    if Config.accountTypesDf is None:
        Config.AccountTypesRead()

    win = AccountTypesPanel(nb, log)
    return win
