...
ifeq ($(TARGET),linux)
  ...
  build/$(CPU_BUILD_DIR)/build/installer/tickerscrape.tgz: build/$(CPU_BUILD_DIR)/TickerScrape$(EXE_SUFFIX)
	...
	cd build/$(CPU_BUILD_DIR); mkdir tickerscrape; cp -a bitmaps data libpython* README.md views bmp_source Format.py Main.py scrape widgets cursors lib src TickerScrape tickerscrape; tar cvfz tickerscrape.tgz tickerscrape; mv tickerscrape.tgz build/installer
